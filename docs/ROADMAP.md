...
(learning/assessment/ai-gateway) are implemented and tested (110 Vitest tests);
the web app has the full adaptive loop working offline-first via IndexedDB, a
live cloud sync channel (Supabase, schema `zivvvo`), and the first-time journey
(Welcome → goal → timeline → confidence → Home) wired end-to-end. Phase 1's
foundation is complete; the current stage is the **backend/sync wrap-up**
(auth + content-pack distribution remain; postgREST exposure of `zivvvo` is an
open environmental gate — ADR-022).
